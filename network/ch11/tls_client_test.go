...
func TestClientTLS(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("expeceted status %d; actual status %d",
	...
	_, err = client2.Get(testServer.URL)
	if err == nil || !strings.Contains(err.Error(), "certificate signed by unknown authority") {
		t.Fatalf("expeceted unknown authority error; actual: %q", err)
	}

	tp.TLSClientConfig.InsecureSkipVerify = true

	resp, err = client2.Get(testServer.URL)
	if err != nil {
		t.Fatal(err)
	}
